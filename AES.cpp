...
bool AES::setKey(const unsigned char* keyArray)
{
	...
	{
		fprintf(stderr, "invalid key: Incorrect length of key");
		exit(-1);
	}
	char* key = new char[16];
	strncpy(key, ((char*)keyArray)+1, 16);
	//first byte tells whether encrypt or decrypt
	//next 16 are the key
	if(keyArray[0] == '0')
	{
		if(AES_set_encrypt_key((const unsigned char*)key, 128, &enc_key)!= 0)
		{
	...
	{
		if(AES_set_decrypt_key((const unsigned char*)key, 128, &dec_key)!= 0)
		{
			fprintf(stderr, "AES_set_encrypt_key() failed!\n");
			exit(-1);
		}
